...
print("output file: "+sys.argv[1]+"/output.log")
with open(sys.argv[1]+"/output.log", "w") as f:
    f.write(f"LOGS DATE: {datetime.now()}\n\n")
# exit()
for x in dirs:
    ...
    with open(sys.argv[1]+"/output.log", "a+") as f:
        f.write(f"Запись: {x} - ")
    print("Peaks:")
    for k in deriative:
        if (k < 0.5):
